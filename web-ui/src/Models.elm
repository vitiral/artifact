...
import Artifacts.Models exposing 
  (Artifact, Artifacts, ArtifactId, NameKey,
  Columns, TextViewState, Search, 
  initialColumns, initialTextViewState, initialSearch)
...
type alias Model =
  { artifacts: Artifacts
  , names: Dict.Dict NameKey ArtifactId -- get the id of a name
  , route: Route
...
nameIds artifacts =
  let
    pairs = List.map (\a -> ( a.name.value, a.id )) (Dict.values artifacts)
  in
    Dict.fromList pairs
...
log model msg =
  let
    ...
    newLogs = { logs | all = List.append logs.all [msg] }
  in 
    { model | logs = newLogs }
