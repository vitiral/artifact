...
use core::types::{Artifact, Artifacts, ArtTypes, ParseFileError, LoadResult};

lazy_static!{
    pub static ref ART_VALID: Regex = Regex::new(r"(REQ|SPC|RSK|TST)[A-Z0-9-]+\z").unwrap();
}

fn artifact_name_valid(name: &String) -> bool {
    ...
    ($value: expr, $attr: expr, $name: expr) => {
        match $value {
            ...
            None => {
                ...
                return Err(ParseFileError::new(String::from_utf8(msg).unwrap()));
            }
        }
    }
}
...
pub fn load_table(artifacts: &mut Artifacts, ftable: &mut Table, path: &Path) -> LoadResult<u64> {
    ...
    let invalid_type = |name: &str, attr: &str| -> ParseFileError{
        let mut msg = Vec::new();
        write!(&mut msg, "{} has invalid attribute: {}", name, attr).unwrap();
        ParseFileError::new(String::from_utf8(msg).unwrap())
    };
...
pub fn load_text(artifacts: &mut Artifacts, text: &str, path: &Path) -> LoadResult<u64> {
    ...
    let mut table = match parser.parse() {
        ...
        None => {
            ...
            return Err(ParseFileError::new(desc));
        },
    };
...
pub fn load_file(artifacts: &mut Artifacts, load_path: &Path) -> LoadResult<u64> {
    ...
    try!(fp.read_to_string(&mut text).or_else(
        |err| Err(ParseFileError::new(err.to_string()))));
...
pub fn recursive_raw_load<P: AsRef<Path>>(load_path: P) -> LoadResult<Artifacts> {
    ...
    if error {
        return Err(ParseFileError::new("ERROR: some files failed to load".to_string()));
    }
    else {
        Ok(artifacts)
    }
